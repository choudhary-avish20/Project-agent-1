...
//fake cursor!
  ...
  async function init() {
    ...
    console.log(`[agent-runtime] fetching token from ${TOKEN_SERVER_URL}/token  room=${roomName}  identity=${identity}`);
    const res = await fetch(`${TOKEN_SERVER_URL}/token?room=${encodeURIComponent(roomName)}&identity=${encodeURIComponent(identity)}`);
    if (!res.ok) {
      console.error(`[agent-runtime] token fetch failed: ${res.status} ${res.statusText}`);
      return;
    }
    ...
    room.on(RoomEvent.DataReceived, async (payload, participant, _kind, topic) => {
      ...
      console.log("[agent-runtime] 📩 Data received from:", participant ? participant.identity : "unknown", "msg:", msg);
